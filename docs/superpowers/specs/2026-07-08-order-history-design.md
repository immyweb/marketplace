...
**`GET /order/:id` (modified)** — gains `requireAuth`, then checks `order.user_id === req.userId` in the service layer. A mismatch (or a signed-out request) returns the existing `NotFoundError` (404), not `ForbiddenError` (403) — a 403 would confirm to an unauthenticated caller that an order ID exists, which a 404 doesn't. This closes the exact gap ADR 006 flagged and is a breaking change to that endpoint's public contract (previously ungated); ADR 006 gets a short update recording that the gap is closed and how (see Consequences).
...
**`app/orders/[id]/page.tsx`** (new) — same three sections as `order-confirmation/[id]` (order summary/items, delivery address, payment last-4), reusing that page's layout and styling conventions but with neutral copy (`Order #123` heading, no "Order Confirmed" banner). Calls `fetchOrder(id)` and, on a 404 (`ApiRequestError` with `status === 404`), calls `notFound()` — same pattern as `order-confirmation/[id]`. Because `GET /order/:id` is now ownership-checked, this single 404 path also naturally covers "not signed in" and "not your order" — no separate handling needed.

**`lib/api.ts`**: add `fetchOrders()` returning `OrderSummary[]`, mirroring the existing `fetchOrder`.
...
- Signed-out access to `/orders` or `/orders/[id]` → redirected to sign-in at the page level (Server Component gate on `/orders`; 404-via-ownership-check on `/orders/[id]`, consistent with how `order-confirmation/[id]` already treats a missing order).
- Direct API calls to `GET /order` without auth → 401/403 via the existing `requireAuth` middleware (unchanged behavior, just newly applied here).
- Direct API calls to `GET /order/:id` for another user's order, or while signed out → 404, not 403, to avoid confirming order existence to a caller who doesn't own it.
...
- `GET /order/:id` returns 404 when signed out (replacing the old ungated-access assumption).
...
- `/orders/[id]`: renders order detail; 404s (via `notFound()`) when the API returns 404.

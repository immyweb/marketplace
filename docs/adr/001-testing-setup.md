...
- **Network is mocked with MSW** (`msw/node`), configured in `tests/component/setup.ts`. Per `CLAUDE.md`, mocking is fine for most things — component tests should stay fast and isolated from the API/DB, with real integration still covered by the API's own Vitest suite and by Playwright e2e for the critical flows (checkout, cart, payment) that require it.
